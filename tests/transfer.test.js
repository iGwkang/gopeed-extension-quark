...
test('取链先完成全部精确匹配，再按大小匹配，等长时剩余按索引对齐', () => {
  const files = [
...
test('条数不等时名称和大小均不匹配则不硬凑直链', () => {
  const matchedLinks = matchDownloadLinks(
    [
      { file_name: 'wanted', size: 10 },
      { file_name: 'another', size: 20 },
    ],
    [{ file_name: 'wrong', size: 99, download_url: 'wrong-url' }],
  );

  assert.deepEqual(matchedLinks, [undefined, undefined]);
});

test('优先按转存 fid 对齐直链', () => {
  const matchedLinks = matchDownloadLinks(
    [
      { file_name: 'a.rar', size: 1 },
      { file_name: 'b.rar', size: 2 },
    ],
    [
      { fid: 'fid-b', file_name: 'x', size: 9, download_url: 'url-b' },
      { fid: 'fid-a', file_name: 'y', size: 8, download_url: 'url-a' },
    ],
    ['fid-a', 'fid-b'],
  );
