...
将夸克网盘分享链接或自己网盘目录链接粘贴到 Gopeed，扩展会递归解析目录并获取下载直链，同时用 `path` 保留原有目录结构。

支持：

- `pan.quark.cn/s/*` 和 `drive.quark.cn/s/*` 分享链接
- `pan.quark.cn/list#/...` 和 `drive.quark.cn/list#/...` 自己网盘目录（含多级 hash）
- URL 中的提取码（`?pwd=`）
- 目录递归与相对路径保留
- 分享链接按网盘可用空间分批转存
- 下载直链过期后自动刷新
...
在 Gopeed 中用链接新建下载任务（不要选择“直接下载”），例如：

```text
https://pan.quark.cn/s/abcdef123456
https://pan.quark.cn/s/abcdef123456?pwd=7x9k
https://drive.quark.cn/s/abcdef123456?pwd=7x9k
https://pan.quark.cn/list#/list/all/{目录fid}-文件夹名
https://pan.quark.cn/list#/list/all/{一级fid}-GopeedTemp/{二级fid}-新建文件夹
```

自己网盘目录链接从浏览器地址栏复制即可；多级路径会取最后一段目录的 fid。自己网盘文件直接取直链，不经过 `GopeedTemp` 转存。

提取码只从分享链接的 `?pwd=` 参数读取。需要提取码的分享若没有该参数，请在链接末尾补充 `?pwd=提取码`。

建议在 Gopeed 的任务设置中将**连接数设为 256**，以改善夸克直链的下载速度；实际速度仍取决于网络和账号限制。

## 临时转存与清理

解析**分享链接**时，扩展会把文件分批转存到你网盘根目录下的 `GopeedTemp`，取得直链后再交给 Gopeed 下载。自己网盘目录任务不会转存。
